...
// group (the server plus anything a wrapper such as `npx` spawned) can be
// signalled without touching BujiCoder's own process group.
...
// The MCP SDK only signals the direct child, so a wrapper process leaves its
// real server process running and holding the pipes. Because setProcessGroup
