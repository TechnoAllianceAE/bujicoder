	...
	case tea.KeyMsg:
		...
		case "enter":
			...
			if m.state == StateChat && !m.streaming && strings.TrimSpace(m.input) != "" {
				...
				if userMsg == "/usage" {
					m.input = ""
					m.messages = append(m.messages, ChatMessage{Role: "assistant", Content: "Usage tracking available with BujiCoder Enterprise. Visit bujicoder.ai for details."})
					return m, nil
				}
...
func renderWelcomeScreen(version, buildTime string, width int, collapsed bool) string {
	...
	b.WriteString("\n")
	b.WriteString(descStyle.Render("  For teams: bujicoder.ai") + "\n\n")
